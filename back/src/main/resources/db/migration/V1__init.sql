...
--     스키마 변경은 V2, V3 ... 새 파일로만 합니다.
--
--  !! executeInTransaction=false 는 필수입니다.
--     CREATE EXTENSION timescaledb 와 연속 집계(candle_1w) 생성은
--     트랜잭션 블록 안에서 실행할 수 없습니다. Flyway 는 기본적으로
--     스크립트 전체를 한 트랜잭션으로 감싸므로 이 지시자로 꺼 줍니다.
--     아래 테이블 생성부는 자체 BEGIN ... COMMIT 으로 원자성을 유지합니다.
-- ============================================================================
...
--
--   · 장 마감 후 10분부터 17:10까지 30분 간격으로 재시도한다.
--     당일 저장 완료 종목은 제외하고, 캘린더 거래일과 응답 날짜가 일치할 때만 저장한다.
...
-- ────────────────────────────────────────────────────────────────────────────
--  14. 가상 호가 버전 (order_book_version)
--      현재가(quote_snapshot)를 기준으로 생성한 공유 가상 호가 버전 헤더입니다.
--      종목당 is_active = true 인 행은 최대 1개이며, DB를 유일한 상태 기준으로 삼습니다.
-- ────────────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────────────
--  15. 가상 호가 레벨 (order_book_level)
--      버전별 매수(BID) 10호가와 매도(ASK) 10호가 총 20개 행입니다.
--      초기 수량(initial_quantity)은 감사용이며, 잔여 수량(remaining_quantity)만 소비됩니다.
-- ────────────────────────────────────────────────────────────────────────────
