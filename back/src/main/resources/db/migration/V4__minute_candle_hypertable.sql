...
-- ────────────────────────────────────────────────────────────────────────────
--  4. 주봉 갱신 창 3개월 -> 15개월(최대 조회 범위 12개월 + 여유) 확대 — V1의 3개월로는 모자란다.
--
--   고려한 다른 방법) 이 마이그레이션 파일에 최초 1회 전체 수동 집계(refresh) 스크립트를 넣어
--   주봉을 미리 만드는 방법도 있다. 이때 윈도우는 1개월로 축소해도 이후 자동 집계는 정상적으로 신규 주봉을 만든다.
--
--   하지만 이 방법은 다음과 같은 트레이드 오프가 있다.
--   1) 수동 집계 스크립트는 트랜잭션 내에서 돌지 않아서 트랜잭션 및 롤백 안정성을 포기해야 한다.
--   2) refreshWeekly가 일봉 적재 시 전체 수동 집계를 하지만 실패 시 로그만 남긴다.
--      창 크기를 15개월로 넓히면 자동 집계가 구멍난 부분을 메워줄 수 있다.
--
--   그리고 자동 집계는 1일마다 돌기 때문에 그 사이 적재가 일어나면 최대 1일간 해당 주봉을 유저가 확인할 수 없다.
--   그래서 refreshWeekly로 적재 시 즉시 집계하도록 한다. 자동 집계는 이 문제를 해결하지 않는다.
-- ────────────────────────────────────────────────────────────────────────────
SELECT remove_continuous_aggregate_policy('candle_1w');

SELECT add_continuous_aggregate_policy('candle_1w',
       start_offset      => INTERVAL '15 months',
