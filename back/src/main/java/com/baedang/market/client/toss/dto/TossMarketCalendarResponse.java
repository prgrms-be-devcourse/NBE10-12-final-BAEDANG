 ...
 *
 * <p>{@code previousBusinessDay}/{@code nextBusinessDay} 는 응답에 존재하지만
 * 이번 스프린트 스코프(오늘 하루의 정규장 오픈 여부)엔 필요 없어서 매핑하지
 * 않습니다 — {@code @JsonIgnoreProperties(ignoreUnknown = true)} 덕분에
 * 무시해도 역직렬화가 깨지지 않습니다. 필요해지면 필드만 추가하면 됩니다.
 *
 * <p><b>{@code today.integrated} 가 {@code null} 이면 휴장일입니다.</b> boolean
 * "오픈 여부" 필드가 따로 없고, null 여부로 판단하는 구조입니다 (Toss 응답 형태 그대로).
 */
@JsonIgnoreProperties(ignoreUnknown = true)
public record TossMarketCalendarResponse(Today today) {
