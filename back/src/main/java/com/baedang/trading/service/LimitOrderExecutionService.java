...
public class LimitOrderExecutionService {
    ...
    public LimitExecutionOutcome execute(Long orderId, LimitExecutionPreparation selected) {
        if (selected == null || !selected.available()) throw new IllegalArgumentException("후보 선정 근거가 필요합니다");
        ...
        for (int retry = 0; retry < 2; retry++) {
            try {
                ...
                LimitExecutionPreparation current = prepare(order.getStockId(), order.getSide());
                if (!current.available()) return deferred(current.reason());
                // 새 호가·환율로 후순위 주문만 재시도하지 않고 워커에서 선순위를 다시 선정합니다.
                if (!current.samePriorityBasis(selected)) return deferred(PRIORITY_CHANGED);
                result = transactions.execute(new LimitExecutionAttempt(order.getAccountId(), orderId, order.getStockId(),
                        order.getExecutionCount(), current.book().version(), current.book().revision(), current.context()));
    ...
    public LimitExecutionPreparation prepare(Long stockId, OrderSide side) {
        try {
            return prepareMarket(stockId, side);
        } catch (PessimisticLockingFailureException | QueryTimeoutException exception) {
            return LimitExecutionPreparation.unavailable(LOCK_BUSY);
        } catch (BusinessException exception) {
            return LimitExecutionPreparation.unavailable(switch (exception.getErrorCode()) {
                case STOCK_STATUS_UNAVAILABLE -> STATUS_UNAVAILABLE;
                case MARKET_CLOSED -> MARKET_CLOSED;
                case EXCHANGE_RATE_NOT_FOUND, MARKET_CONTEXT_EXPIRED -> CONTEXT_EXPIRED;
                default -> throw exception;
            });
        }
    }

    private LimitExecutionPreparation prepareMarket(Long stockId, OrderSide side) {
        Stock stock = stocks.findById(stockId).orElseThrow(() -> new BusinessException(ErrorCode.INTERNAL_ERROR));
        ...
        if (!session.open() || session.validUntil() == null || !clock.instant().isBefore(session.validUntil())) return LimitExecutionPreparation.unavailable(MARKET_CLOSED);
        LimitExecutionBook book = books.read(stock, side, clock.instant()).orElse(null);
        if (book == null) return LimitExecutionPreparation.unavailable(NO_BOOK);
        stock = statuses.requireCurrent(stock);
        if (policy.determineStaticRejection(stock) != null) return LimitExecutionPreparation.unavailable(NOT_TRADABLE);
        ...
        return LimitExecutionPreparation.available(stockId, side, book, context);
    }
