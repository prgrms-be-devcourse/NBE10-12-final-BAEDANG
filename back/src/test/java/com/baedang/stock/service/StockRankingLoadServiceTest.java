...
class StockRankingLoadServiceTest {
    ...
    private final StockRankingLoadService service =
            new StockRankingLoadService(rankingPort, stockRepository, self);
    ...
    class Load {
        ...
        void loadAll_은_두_시장을_모두_적재한다() {
            ...
            when(rankingPort.fetchRanking(MarketCountry.KR))
                    .thenReturn(new RankingSnapshot(kr, OffsetDateTime.now()));
            when(rankingPort.fetchRanking(MarketCountry.US))
                    .thenReturn(new RankingSnapshot(us, OffsetDateTime.now()));

            service.loadAll();

            verify(self).applyRanking(MarketCountry.KR, kr);
            verify(self).applyRanking(MarketCountry.US, us);
        }
    }
    ...
    class ApplyRanking {
        ...
        void 새_랭킹에_없는_종목은_빠진다() {
            ...
            service.applyRanking(MarketCountry.KR, List.of(엔트리(1, "005930")));
        ...
        void 심볼을_정규화해_매칭한다() {
            ...
            service.applyRanking(MarketCountry.US, List.of(엔트리(1, " aapl ")));
