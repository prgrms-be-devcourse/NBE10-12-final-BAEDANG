...
export default function MainPage() {
  ...
  // .06 → 호버 시 .24)을 그대로 가져왔다. 처음엔 라이트 모드에서 흰
  // 배경 위에 흰색 반투명 배경이 잘 안 보일까봐 라이트 모드만 잉크색
  // 반투명(rgba(15,56,104, ...))으로 바꿨었는데, "투명한 느낌이 나는
  // 흰색"으로 해달라는 요청에 따라 라이트/다크 모드 모두 흰색 반투명으로
  // 통일했다 — 어차피 이 버튼이 놓인 히어로 카드 배경은 흰색이 아니라
  // 옅은 하늘색(라이트)/짙은 남색(다크)이라 흰색 반투명이 두 모드 모두에서
  // 옅게 비쳐 보인다.
  const pillBtnBg = "rgba(255,255,255,.06)";
  const pillBtnBgHover = "rgba(255,255,255,.24)";
