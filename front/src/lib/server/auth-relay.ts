...
const ACTIONS = new Set(['signup', 'login', 'refresh', 'logout', 'password/forgot', 'password/reset']);
...
export async function relayAuth(request: NextRequest, action: string) {
  ...
  try {
    ...
    if (action === 'password/forgot' || action === 'password/reset') {
