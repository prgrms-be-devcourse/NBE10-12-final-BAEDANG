...
export const COMPARE_TITLE_WORDS = ['기존', '증권사', '서비스와', '무엇이', '다른가요?'] as const;
 ...
 * 단어 배열로 나눴다. "5,000만원"에는 파란 그라데이션이 훑고 지나가는
 * 효과를 따로 적용해야 해서 뒤에 붙는 조사 "으로"를 별도 단어로 뗐다 —
 * 둘 사이는 공백 없이 붙고("5,000만원으로"처럼 보인다), "으로" 다음에서
 * 줄바꿈된다(컴포넌트에서 이 단어들을 기준으로 붙이거나 <br/>을 넣는다). */
