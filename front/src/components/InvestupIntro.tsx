...
export function InvestupIntro({
  ...
  useEffect(() => {
    ...
    const paintGlobe = (dt: number, now: number) => {
      ...
      // 끝은 투명, 아래쪽 끝은 진하게).
      // 옅어지는 범위를 더 넓게 → 좀 더 많이 넓게 해달라는 요청을 두
      // 차례 거쳤다 — 위쪽 끝(topY)은 그대로 두고, 진한 색이 되는
      // 아래쪽 끝(botY)을 보이는 캔버스 바닥(f.h)보다 더 아래로 늘렸다
      // (1.7배 → 2.8배). 그러면 실제로 보이는 캔버스 영역(0~f.h)은
      // 전체 그라데이션 중 더 작은 일부(botY 이전)만 차지하게 돼서,
      // 화면에는 최대 진하기(0.45)까지 훨씬 못 미친 채 더 넓은 구간에
      // 걸쳐 천천히 옅어지는 것처럼 보인다.
      const topY = Math.max(0, g.cy - g.R);
      const botY = topY + (f.h - topY) * 2.8;
