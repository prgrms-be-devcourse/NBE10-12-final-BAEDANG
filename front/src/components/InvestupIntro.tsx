    ...
    >
      ...
      >
        ...
        >
          ...
          <img
            ...
            style={{
              ...
              width: '100%',
              height: 'auto',
              // 왼쪽·위쪽·아래쪽이 옅어지지 않아 부자연스럽다는 피드백 —
              // 원인을 찾아보니 타원 크기(62%)가 50%를 넘어서 있었다.
              // radial-gradient의 크기를 퍼센트로 주면 그 값이 박스
              // 가장자리까지의 거리(=50%)를 기준으로 삼는데, 62%처럼
              // 50%보다 큰 값을 주면 실제 이미지 가장자리(딱 50% 지점)
              // 에 도달했을 때 그라데이션이 아직 100%(완전 투명)까지
              // 못 가서, 가장자리에 옅게나마 색이 남아있게 된다 — 그게
              // 왼쪽·위쪽·아래쪽에서 보이던 부자연스러운 경계였다(오른쪽·
              // 아래쪽 일부는 사진 자체가 이미 밝아서 눈에 덜 띄었을
              // 뿐이다). 타원 크기를 50% 밑으로(46%) 낮춰서 실제
              // 가장자리에 닿기 "전에" 이미 완전히 투명해지도록 여유를
              // 뒀고, 불투명 구간도 8%로 더 줄여서("넓은 그라데이션"
              // 요청대로) 거의 전체가 서서히 퍼지듯 옅어지는 폭넓은
              // 그라데이션이 되게 했다.
        ...
        {/* 히어로 화면(로고 + "실수는 가볍게, 투자 감각은 제대로") 상단에
            ...
            rgba(196,222,248))을 재사용했는데, 이후 "제가 첨부한 사진 속
            색상으로 바꿔달라"는 요청으로 사진 속 하늘색(더 또렷하고 채도
            있는 파랑, rgb(140,189,241) ≈ #8cbdf1)으로 다시 바꿨다. 카드
            쪽은 중심에서 퍼지는 radial-gradient였지만, 여기서는 "위에서
            시작해 아래로 갈수록 옅어지는" 세로 방향으로 달라는 요청이라
            linear-gradient(180deg, ...)로 바꿨고, 사진처럼 흰색으로
            끝나면 히어로 배경 전체를 흰 판으로 덮어버리므로 마지막
            지점만 투명(alpha 0)으로 바꿔 히어로 자체 배경 위에 자연스럽게
            녹아들게 했다(색상 스톱 구조·비율은 이전과 동일하게 유지).
        ...
        <div
          ...
          style={{
            ...
            pointerEvents: 'none',
            background:
              'linear-gradient(180deg, rgba(140,189,241,0.85) 0%, rgba(140,189,241,0.4) 45%, rgba(140,189,241,0) 100%)',
            opacity: lifted ? 0 : 1,
            transition: 'opacity 1.6s cubic-bezier(.16,1,.3,1)',
          }}
        />
          ...
          {pin.on && (
            <div
              style={{
                ...
                borderRadius: 20,
                // 지구본 핀 호버 팝업을 좀 더 투명하게 해달라는 요청을 네
                // 차례 거쳤다 — 두 stop의 알파값을
                // 0.58→0.4→0.28→0.15→0.09, 0.4→0.26→0.16→0.08→0.04로
                // 계속 낮췄다. "뒤의 지구본이 약간 비칠 정도로" 해달라는
                // 요청이라, 알파뿐 아니라 backdrop-filter의 blur도
                // 18px→10px→7px로 줄였다 — 블러가 강하면 알파를 아무리
                // 낮춰도 뒤 배경이 뭉개진 안개처럼만 보이고 지구본의 도트
                // 패턴은 알아볼 수 없기 때문이다.
                background:
                  'linear-gradient(155deg, rgba(255,255,255,0.09) 0%, rgba(239,246,252,0.04) 100%)',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 6 }}>
                ...
                <span
                  style={{
                    ...
                    alignItems: 'center',
                    justifyContent: 'center',
                    // 체크 아이콘 배경색 — "학습과 훈련"(#1f3a68)과
                    // "기존 증권사 서비스"(#1b6da3)의 중간값(#1d5486)을
                    // 적용했더니 은근히 청록빛이 돈다는 피드백을 받았다.
                    // #1d5486의 색상각(hue)을 계산해보니 약 208.6°로,
                    // 청록(약 180°)에 상당히 가까운 파랑이라 청록기가
                    // 도는 게 맞았다 — 채도(약 64%)·명도(약 32%)는 그대로
                    // 두고 색상각만 순수한 파랑 쪽인 222°로 옮겨서
                    // (HSL(222°, 62%, 32%) → RGB) 청록기 없는 파랑
                    // #1f3d84로 다시 계산했다.
                    background: '#1f3d84',
                    opacity: cmpOpen ? 1 : 0,
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              zIndex: 3,
              // CTA 문구 배경(이 패널 + 그 위 검정 그라데이션)의 기본
              // 남색을 좀 더 세련되고 짙게 해달라는 요청 — 공용 토큰
              // T.deepInk(#0f3868)를 그대로 바꾸면 이 패널과 무관한 다른
              // 곳(위 확대 문구의 글자색 등)까지 같이 바뀌어버려서, 이
              // 패널 전용 색을 새로 만들었다. 같은 색상(hue)·채도
              // (saturation) 계열은 유지한 채 명도만 크게 낮춰(23%→16%)
              // 더 짙어 보이게 했다.
              background: '#0a2647',
              borderRadius: 16,
              overflow: 'hidden',
          ...
          {/* 배경 중앙에 남색 패널 기본색보다 더 어두운 색이 퍼지는
              ...
              투명해져 남색 패널 기본색(#0a2647)과 자연스럽게 이어진다. */}
          <div
            aria-hidden="true"
            style={{
              position: 'absolute',
