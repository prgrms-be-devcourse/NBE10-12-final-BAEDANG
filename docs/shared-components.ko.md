...
가상 호가 V1 설정값은 다음과 같이 고정합니다: `enabled=false`, `policyVersion=V1`, `refreshInterval=3s`, `refreshInitialDelay=0s`, `levelsPerSide=10`, `spreadStepsPerSide=1`, `maxQuoteAge=15s`, `krBaseNotional=20000000`, `usBaseNotional=15000`, `minQuantity=1`, `maxQuantity=1000000`, `noiseMinBps=8000`, `noiseMaxBps=12000`, `unconsumedRetention=1m`, `retentionInitialDelay=0s`. 이 값은 모의 공급 정책이며 실제 시장 잔량 재현값이 아닙니다. 두 initial delay는 스케줄러 시작 시점만 제어하는 운영 설정이며 0 이상이어야 합니다.
