...
- 접수 커밋 후 알림을 발행합니다. 새 주문이 저장된 커서보다 선순위면 해당 방향을 처음부터 재평가하고, 후순위면 커서는 유지하되 읽어 둔 페이지를 무효화합니다. 롤백/거절/멱등 재요청은 진행 위치를 무효화하지 않습니다. 실행 중이던 결과가 무효화된 커서를 덮어쓰지 못합니다. 우선순위는 후보 선정 시 조회 가능한 커밋된 주문을 기준으로 하며 이후 접수 때문에 이미 선정·체결된 결과를 되돌리지 않습니다. 단일 인스턴스의 메모리 진행 정보는 주문별이 아닌 종목·방향별이며 전체 그룹 순회 후 관찰되지 않은 그룹을 제거합니다. 재시작 시 선순위부터 안전하게 다시 시작합니다.
