...
폐기 커밋 뒤 시작하는 인증 검증은 기존 Access와 Refresh를 거절합니다. 이미 인증된 요청을 소급 취소하지는 않습니다. V17의 `token_version` 증가는 호환용이며 실제 폐기는 V18의 `auth_session`으로 처리합니다. [상세 인증 정책](authentication.ko.md)을 참고하세요.
