...
```
DB에서 버킷별 마지막 원본만 선택합니다. `1d`는 1시간, `1w`/`1m`/`3m`은 1일, `1y`는 7일 버킷입니다. 기존 차트와 동일한 UTC Unix epoch 정렬이며 7일 버킷은 ISO 주가 아닙니다. 요청 시작부터 현재까지 조회하고 빈 버킷은 생략합니다. 선택한 원본 `validFrom`과 반올림하지 않은 표시 환율을 보존합니다. 최대 응답은 각각 25/8/32/93/54개이며 분 단위 원본 전체를 브라우저에 전송하지 않습니다.
