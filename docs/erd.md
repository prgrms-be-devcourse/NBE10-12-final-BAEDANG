...
GET /api/v1/candles?symbol=005930&interval=1d&count=200
     └ KR 15:40 / US 05:10 (06:10 in winter) · 100 calls per market · serialized at 5 TPS
        ↓ store closePrice
...
| `GET /api/v1/candles` (interval=1d) | MARKET_DATA_CHART · **20 TPS** | KR 15:40 / US 05:10 (06:10 in winter) | `daily_candle`. Daily and minute candles share one serialized task queue for this call group. The finalized `close_price` is copied to `quote_snapshot.prev_close` before the next session. Convert timestamps to a KST date. |
...
| Column | Type | Description |
|---|---|---|
| `stock_id` + `trade_date` | composite PK | one row per stock×trading day. Convert the response timestamp to the KST date. |
