...
16. **Market-order button = immediate-fill single transaction** — deposit/quantity change + holding update + `FILLED` order + ledger record in one transaction. It does not pass through `PENDING` or touch reservation fields. Lock the account row with `SELECT … FOR UPDATE` to prevent double-deduction on concurrent orders; roll back entirely on technical failure. A valid business rejection is stored as `REJECTED` without balance or ledger changes. Only future limit orders use separate reservation and fill transactions. Design an idempotency key (`client_order_id`) for duplicate-click protection.
    **Trading eligibility** — both ranked and non-ranked stocks can trade after status, regular-session and source-quote freshness checks. Ranking membership controls scheduled collection, not order eligibility.
17. **Order-disable must show a reason** — market closed / suspension / insufficient deposit / insufficient holdings / stale quote. A disabled button alone leaves users guessing why.
    **Only blocking states go on the button.** The button reason is resolved in a fixed order: `SUSPENDED → LIQUIDATION → active circuit breaker → other tradable reasons → insufficient deposit/holdings`. Market and limit orders share one resolver so the two buttons cannot disagree.
    **Non-blocking states go next to the stock name instead.** Buy-cautions (per-type labels — `과열종목` · `투자경고` · `변동성완화장치`, or `거래유의종목` for an unmapped Toss code) and an active sidecar (`사이드카 매수/매도 발동 중`) render as badges beside the stock name and never disable the button — a sidecar suspends program quotes only. One label for every caution would hide *why* the stock is flagged, and their restrictions differ (투자경고 blocks margin trading and requires full cash upfront). An active circuit breaker has **no** badge: the disabled button already carries `서킷브레이커 발동 중이에요`, and stating it twice reads as two different problems.
    **Caution-lookup failure must not look like "clean".** The API reports `warningsStatus: UNAVAILABLE` separately, and the screen shows no caution badge rather than claiming the stock has none.
...
The LIMIT input displays the available KR range or explains that daily limits are pending; US displays no price restriction. Server rejection reasons distinguish unavailable bounds, out-of-range input and invalid ticks. Short or empty book sides render normally, with an explicit no-ask/no-bid waiting message. Input changes and updated detail bounds trigger a fresh LIMIT estimate after a 500ms debounce. Only `PRICE_LIMIT_UNAVAILABLE` and `QUOTE_OUT_OF_PRICE_LIMIT`, whether returned as estimate reasons or HTTP error codes, enable 5-second recovery polling through `useVisiblePolling`. Polling stops in hidden tabs, when the form is ineligible, or when the response no longer contains either transient error. Successful estimates and input errors do not poll. In-flight requests are deduplicated per input, and late responses from previous inputs or an unmounted form are ignored.
