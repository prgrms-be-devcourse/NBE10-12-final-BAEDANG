...
17. **Order-disable must show a reason** — market closed / suspension / insufficient deposit / insufficient holdings / stale quote. A disabled button alone leaves users guessing why.
    ...
    **Non-blocking states go next to the stock name instead.** Buy-caution (`거래유의종목`, from the stock's Toss cautions) and an active sidecar (`사이드카 매수/매도 발동 중`) render as badges beside the stock name and never disable the button — a sidecar suspends program quotes only. An active circuit breaker has **no** badge: the disabled button already carries `서킷브레이커 발동 중이에요`, and stating it twice reads as two different problems.
