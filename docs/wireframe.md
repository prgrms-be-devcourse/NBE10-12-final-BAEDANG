...
16. **Buy button = single transaction** — deposit deduction + holding increase + ledger record in one transaction. Lock the account row with `SELECT … FOR UPDATE` to prevent double-deduction on concurrent orders; roll back entirely on failure. Design an idempotency key (`client_order_id`) for duplicate-click protection.
