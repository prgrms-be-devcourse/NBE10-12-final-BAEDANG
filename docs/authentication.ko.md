...
NPMplus의 외부 백엔드 주소는 `https://54.180.217.176.sslip.io`이며 내부 `trading-back-1:8080`으로 HTTP 중계합니다.
`AUTH_BACKEND_URL`과 `NEXT_PUBLIC_API_BASE_URL`에는 이 HTTPS 주소를, `AUTH_PUBLIC_ORIGIN`에는
`https://nbe-10-12-final-baedang.vercel.app`을 설정한 뒤 프론트·백엔드를 함께 배포해야 합니다.
...
V18에서 V17의 `token_version` 컬럼을 제거하며 인증은 `auth_session` 활성 상태만 검사합니다.

현재 Refresh의 SHA-256 해시·세대가 일치하면 한 번 회전합니다. 회전 시점부터 **20초 동안** 직전 토큰은
**동일한 후속 Refresh**를 반환하며 회전하거나 유예를 연장하지 않습니다. 후속 토큰은 별도 키의
AES-256-GCM 암호문으로 보관하고 무작위 nonce와 세션 ID AAD를 사용합니다. 20초 경계 이후 또는
...
탭 내 공유만 적용하며, 20초 유예로 임의의 탭 간 동시 요청까지 보장하지 않습니다.
...
V18은 auth_session을 추가하고 users.token_version을 제거하며 기존 회원·계좌·거래 데이터를 보존합니다. 배포 전 토큰은 재로그인이
필요합니다. 프론트만 되돌리는 롤백은 RTR 계약과 호환되지 않으며, token_version을 요구하는 이전 백엔드로 되돌리려면 별도 스키마 복구가 필요합니다. 운영 환경변수 설정과 동시 배포 후
...
재설정 메일 작업은 토큰 발급 트랜잭션 커밋 후에만 등록하며 롤백되면 발송하지 않습니다. 큐 포화·실행기 종료는 토큰·링크 없이 로그에 남기고 응답을 바꾸지 않습니다. 등록 실패가 커밋된 토큰이나 쿨다운을 되돌리지는 않으며, 기존 쿨다운 이후 다시 요청할 수 있습니다. 영속 재시도 큐 없이 최선 노력 방식으로 발송합니다.
