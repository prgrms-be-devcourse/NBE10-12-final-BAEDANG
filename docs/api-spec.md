...
- Worker uses a dedicated single thread, fixed delay 3s, pages of 50, up to 100 order attempts per tick and a 5s budget for starting new orders. No enable/disable flag. A running financial transaction completes even after the budget; a logical attempt includes at most one retry for book changes/lock contention.
- Cycle orderId upper bound and keyset cursors survive tick boundaries. Within a stock/side: BUY limit descending, SELL ascending, then orderedAt/orderId ascending. New IDs enter the next cycle; this is priority among observed candidates, not a database-wide snapshot of late commits.
- Expired/inactive/off-session/no-book candidates do not fetch FX. Status uses the shared 5-minute cache outside the transaction. A normal reserve/non-positive-net deferral allows the next order; unresolved book/lock/context/status failures skip the same stock/side for this cycle, without blocking other groups.
