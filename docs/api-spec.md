...
- Acceptance publishes an after-commit notification. A new order ahead of the saved cursor resets that direction to the head; one behind it preserves the cursor but invalidates the already-fetched page. Rollbacks/rejected requests/idempotent replays do not invalidate progress. An in-flight result cannot overwrite invalidated progress. Priority is among committed orders visible when selecting a candidate; later arrivals do not undo already selected or committed fills. Single-instance in-memory progress is per stock/side, not per order, and unobserved groups are removed after a complete group rotation; restart safely begins at the head.
