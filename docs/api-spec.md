...
Once a `REJECTED` response has been received, that attempt is final. A retry intended to place the order again after conditions change—such as the market reopening, refreshing a stale quote, or adding cash—must use a **new `clientOrderId`**. Reusing the rejected attempt's ID returns the stored rejection and never re-evaluates market conditions.
...
```
The order response contains only the cash balance authoritatively finalized by the transaction. Portfolio valuation is intentionally separated from execution; fetch `GET /accounts/me` when the latest `totalAsset` is needed.
...
`STALE_QUOTE` rejects a quote older than 15 seconds. `FUTURE_QUOTE` rejects any quote timestamp later than the server's validation time. Filling from either timestamp breaks the ledger's credibility.
