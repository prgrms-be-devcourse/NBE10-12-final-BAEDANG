...
- `LimitExecutionProgress`: single-instance stock/side price cursors keyed by bookVersion/numeric FX, not revision. `LimitOrderTransactionService` publishes `LimitOrderAcceptedEvent`; the AFTER_COMMIT listener invalidates a fetched page and resets the cursor only if the new order precedes it. Tokens prevent an in-flight attempt from restoring old progress. The worker prunes unobserved groups after full rotation; do not treat this as a multi-instance lock or persistent order state.
