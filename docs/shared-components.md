...
| [SchedulingConfig](../back/src/main/java/com/baedang/global/config/SchedulingConfig.java) | Provides common `taskScheduler`, dedicated `limitOrderTaskScheduler`, `orderBookTaskScheduler`, and `leaderboardTaskScheduler`, plus `dailyCandleTaskExecutor`. Inject that executor with `@Qualifier("dailyCandleTaskExecutor")` | Order-book refresh and retention are serialized on the one-thread order-book scheduler, separate from common batches, and their DB transactions use a local 2-second lock timeout. Daily candles use one thread, queue capacity 10, and up to 30 seconds for shutdown. Leaderboard shutdown drops pending scheduled runs and immediately removes cancelled reservations from the queue; an already running batch can finish within the existing 30-second shutdown wait. Do not indiscriminately share these executors; each scheduler owns its activation conditions |
...
- Daily candles and quotes use exchange-local trading dates. Actual ledger instants remain UTC and can display in KST. Daily/weekly API KST-midnight timestamps encode date labels, not bar-opening instants. FX reference dates retain their KST policy.
...
| [LatestCompletedTradingDayResolver](../back/src/main/java/com/baedang/market/service/LatestCompletedTradingDayResolver.java) | `resolve(country)` / `resolve(country, requestedAt)` → `Optional<LocalDate>` | Resolves the latest completed trading day using local dates and the calendar; currently a 10-minute finalization delay and up to 14 days of lookback. Returns empty on lookup failure, response mismatch, or no result |
...
| [OrderBookGenerator](../back/src/main/java/com/baedang/orderbook/service/OrderBookGenerator.java) | `generate(policy, stock, basePrice, quoteAt, generatedAt, seed, limits)` | Single V2 generator. Uses `OrderBookPricePolicy` and verified daily bounds for 0–10 levels per side; quantity noise is unchanged |
| [OrderBookExecutionStore](../back/src/main/java/com/baedang/orderbook/port/OrderBookExecutionStore.java) | `lockForExecution(stockId, expectedBookVersion, expectedRevision, side)` | MANDATORY. Locks version and requested-side levels, then validates DB daily bounds and the V2 price array. BUY→ASK, SELL→BID; valid empty sides are supported |
| [OrderBookProperties](../back/src/main/java/com/baedang/orderbook/config/OrderBookProperties.java) | `policyVersion()`, `krBaseNotional()`, `minQuantity()`, etc. | `trading.orderbook` validated runtime properties record. V2 defaults: 3s refresh, 15s maxQuoteAge, 1m retention |
...
The current implementation uses these configurable V2 virtual-order-book defaults: `policyVersion=V2`, `refreshInterval=3s`, `refreshInitialDelay=0s`, `maxQuoteAge=15s`, `krBaseNotional=20000000`, `usBaseNotional=15000`, `minQuantity=1`, `maxQuantity=1000000`, `noiseMinBps=8000`, `noiseMaxBps=12000`, `closedVersionRetention=1m`, and `retentionInitialDelay=0s`. V2 book shape is a code invariant, not runtime configuration: 0–10 levels per side, with one valid tick between neighboring levels, stopping at KR daily limits or positive storable-price boundaries. Closed versions and levels are deleted after retention regardless of consumption; execution price, quantity, and settlement amounts remain permanent in `trade_execution`. A different shape requires a new policy version. These synthetic supply values require rationale and agreement in the #121 PR; implementation alone proves neither policy approval nor empirical market-depth fidelity. The two initial delays control scheduler startup timing only and must be nonnegative.

Virtual order-book publication and query are always enabled. Publication runs only during regular sessions with valid quotes. LIMIT admission, cancellation, expiration and execution remain separate use cases. Without a usable book, the worker defers orders rather than fabricating liquidity.
...
The virtual order book module (`com.baedang.orderbook`) supplies a shared synthetic V2 order book with up to 10 levels per side based on Toss current price and provides the locking store contract for the #122 matching engine.
...
- **Integer Quantity Policy**: Although the database maintains `NUMERIC(19,6)` for future fractional compatibility, V2 synthetic order book generation and fill consumption operate strictly in **whole integer shares** (`minQuantity=1`, `maxQuantity=1000000`).
- **Nature of Quantity Distribution**: Depth multipliers and round-number boosts represent mock market V2 supply policies rather than an empirical replication of real market depth. Never treat upper-level volume dominance as an invariant.
...
Inject QuoteRefreshCoordinator and call requireFresh(stock, maxAge) **outside a transaction** for trading-oriented reads. It reuses a recent source quoteAt or refreshes and rejects missing/future/stale results. It does not open a trade or backfill candles/FX/status. Quote persistence consults the shared calendar; reference recovery uses freshly fetched and validated daily candles. OrderMarketDataService connects it to orders; order orchestration validates the other contracts. refresh(stock) is the display-oriented refresh operation; its caller may explicitly retain an older display value on failure. Existing daily-backfill policy is unchanged.

QuoteSnapshotPersistenceService validates currency, price and regular-session source time outside a transaction, then saves quotes in a short JDBC transaction. Reference recovery separately validates freshly fetched daily closes and updates the reference only while the quote remains on the same exchange-local trading day. Newer ticks within that day do not prevent recovery. New trade dates clear old references; same-date verified references and price limits survive missing enrichment. Older responses cannot overwrite newer verified quotes. V10 adds only prev_close_date.
...
- `ExecutionExchangeRateProviderBridge`: normal reads use only the latest DB row without a memory cache. MarketOrderService alone invokes `refreshUnavailableForMarketOrder()` on missing/expired FX, outside financial transactions, then rereads the DB once. Uses `rate`, validates the source validity window and future receipt time; unavailable FX is not replaced with an expired row. Financial transactions recheck the supplied snapshot after locking. `ExchangeRateService` and account valuation share the DB but retain their `midRate`-first display policy. Latest/history APIs expose `validFrom`, and the frontend polls latest every minute. Scheduled collection and recovery share ExchangeRateLoadService's shared future on the dedicated single-thread exchangeRateRefreshExecutor. Each caller, including the first, waits up to trading.exchange-rate-refresh-wait (default 5s), including task queue/API time. A timeout does not cancel shared work; later requests can use its eventual result. Calls are suppressed for 5 seconds after completion (not an FX TTL). Failure or still-expired data returns SAME_CLIENT_ORDER_ID; no startup collection is added.
...
- `DailyCandlePersistenceService.upsert(stockId, currency, country, candles, requestedAt)`: validates finalization at request start and returns the accepted `List<DailyCandle>`. Recovery selects exact dates from that result, not pre-existing DB rows. Run the fetch and write within the shared coordinator.
- `PrevCloseUpdateService.update(country)` / `recover(stock)`: obtains the latest finalized daily close and its exact previous trading day's close. Background targets are ranked/active-limit stocks; detail recovery also covers non-ranked stocks. A recovered closing price uses the calendar's regular close as `quote_at`; it cannot overwrite a newer regular-session quote.
- `trading.reference-recovery.initial-delay=5s`, `interval=1m`: startup catch-up and retries on the dedicated `referenceRecoveryScheduler` thread. All daily-candle fetch/write paths share bounded striped locks. Failed references never copy `last_price`.
- `MinuteCandlePersistenceService.upsert(stockId, country, candles)`: stores only bars whose opening timestamp lies in the regular session `[open, close)`.

`MarketStatusService` also resolves each market using its local date and reuses the calendar Port cache. Response timestamps retain their KST presentation.

`QuoteSnapshotPersistenceService.repairReference(stock, expected, reference)` accepts only a candle validated in the current external fetch. It checks the preceding trading day and updates reference fields only while the exchange-local date derived from quote_at still matches (including newer ticks within that day, but excluding the next trading day); price and collection time are preserved. `saveRecoveredClose` replaces a closing price only if the observed snapshot is unchanged.

`DailyCandleFetchCoordinator.withStockLock(stockId, operation)` is shared by scheduled collection, seeding, detail backfill and reference recovery. Serialize the entire external fetch through committed storage for the same stock. Locks cover one application instance; multiple collector instances require separate distributed coordination.


### Shared calendar policy and daily-candle completion

- Calendar consumers use `MarketTradingDayPolicy.calendar(country, date)` for country routing and response validation. The cache uses the same `MarketCalendarDay.requireMatching` validation before insertion, so mismatched country/date responses are not cached. Callers retain their own failure handling (skip a batch or propagate an exception).
- `MarketCalendarDay.isRegularSessionAt(instant)` checks `[open, close)` for session status and minute-bar starts. `acceptsRegularQuoteAt(instant)` also accepts a quote at the exact closing instant. `isFinalizedAt(requestedAt)` shares the close-plus-10-minutes rule; pass external request start time when validating daily candles.
...
- SELL admission in both transaction services repeats `activeFor(stock, now)` after the holding lock and before the second context-freshness check. A CB committed during that lock wait produces the same audited rejection, even if context/session/quote validity also expired. BUY admission retains the account-lock check without an additional lookup for this sell-only lock boundary.
...
Detail collection uses `ensureForDisplay(stock, existingQuote)` and returns the snapshot to display. US stocks, disabled collection and snapshots with today's limits reuse the supplied snapshot without another SELECT. When collection is needed, reuse the snapshot read inside the service; after a save attempt, reread even if UPDATE affected zero rows so another request's accepted limits are returned. A verified current-day reread counts as success. Collection failure retains the last snapshot read, or the supplied snapshot if no read succeeded. If the price-limit gate cannot grant immediately, return stored data without queuing or recording a failure cooldown. Background `ensure` continues to wait for the shared 2 TPS gate and does not perform a display reread. An admitted request still uses the existing broker group limiter and HTTP timeout; this does not make the entire detail endpoint asynchronous.

The attempts map and mutable attempt fields are guarded by the same `synchronized` claim/finish methods. Keep expiry cleanup, duplicate/capacity checks and registration atomic; external API calls remain outside that monitor.

### Display state for stock detail

- `StockWarningQueryService.currentWarnings(stock)`: reads the Toss caution endpoint through `SymbolInfoPort.fetchStockWarnings` behind a bounded per-stock TTL cache (`trading.stock-warning-cache-ttl`, default 5m). Caches the **raw** cautions, not the filtered result — the active window is re-evaluated against the read date so a caution whose `endDate` passed at midnight disappears even while the cache entry is still fresh.
- Returns `warningsStatus`: `UNAVAILABLE` means the lookup failed and there was no earlier confirmed value. Never treat that as "no cautions" and never fold it into `tradable` — a caution is informational and must not change order eligibility.
- `StockDetailService` renders `warnings`/`warningsStatus`; suspension and liquidation still come from `StockTradingState` and `listingStatus`, not from cautions.
- Caution labels are per type, mapped in the backend from `tools/terms.md` (`OVERHEATED` → `과열종목`, `INVESTMENT_WARNING` → `투자경고`, `VI_STATIC` → `변동성완화장치`); an unknown code falls back to `거래유의종목`. The raw code stays in `type`. The frontend renders the server's `label` as-is — translating the same fact in two places lets one side drift.
- Frontend: `front/src/lib/stock-status.ts` decides what the stock-detail screen shows. `buildStatusBadges` produces only **non-blocking** badges (per-type caution labels, active sidecars with direction) and `resolveBlockReason` produces the single button reason in the order `SUSPENDED → LIQUIDATION → active circuit breaker → other tradable reasons → amount/quantity`. Both order types call the same function, so market and limit orders cannot disagree.
