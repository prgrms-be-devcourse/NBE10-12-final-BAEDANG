...
- `LimitOrderExecutionService.execute(orderId)`: NEVER boundary; prepares cached status, session and execution FX, then invokes the database service. Book/lock conflicts get at most one fresh preparation retry.
