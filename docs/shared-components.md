...
QuoteSnapshotPersistenceService validates currency, price and regular-session source time outside a transaction, then saves quotes in a short JDBC transaction. Reference recovery separately validates freshly fetched daily closes and updates the reference only while the quote remains on the same exchange-local trading day. Newer ticks within that day do not prevent recovery. New trade dates clear old references; same-date verified references and price limits survive missing enrichment. Older responses cannot overwrite newer verified quotes. V8 adds only prev_close_date.
