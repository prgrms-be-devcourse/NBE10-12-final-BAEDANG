...
- V13 adds only `price_limit_date`, preserving existing rows. Deployment must coordinate pending earlier migrations; this branch does not invent or copy V9/V11/V12.

Detail collection uses `ensureForDisplay`: if the price-limit gate cannot grant immediately, return stored data without queuing or recording a failure cooldown. Background `ensure` continues to wait for the shared 2 TPS gate. An admitted request still uses the existing broker group limiter and HTTP timeout; this does not make the entire detail endpoint asynchronous.
