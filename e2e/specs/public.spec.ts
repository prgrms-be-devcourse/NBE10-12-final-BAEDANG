...
for (const next of ['/my', '/main']) {
  test(`회원가입 화면에서 약관 동의 후 새 계좌를 생성한다 (${next})${next === '/my' ? ' @smoke' : ''}`, async ({ page }) => {
    await page.goto(next === '/my' ? '/signup?next=/my' : '/signup');
    ...
    await expect(page).toHaveURL(`http://127.0.0.1:13000${next}`);
    if (next === '/main') await page.goto('/my');
